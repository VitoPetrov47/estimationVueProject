  ...
  <div class="wrapper-main">
    <div class="container-custom-L">
      <section class="section-block card-section">
        <h1 class="card-title">Information Project Parameters</h1>

        <div class="card-content-main">
          <div class="card-content">
            <label class="card-text">Type of the project</label>
          ...
          <div class="card-content">
            <label class="card-text">Platform</label>
            <div class="custom-select ">
      ...
      <footer class="footer-info-main">
        <button class="btn footer-save-next-btn" @click="submitProjectInfo">Save and Next</button>
        <label class="footer-step-text">Next step: Attribution</label>
      </footer>
    </div>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const clientNickname = ref(route.query.clientNickname || '');
    const PM = ref(route.query.PM || '');
    const milestone = ref(route.query.milestone || '');
    ...
    onMounted(() => {
      ...
      console.log('Client Nickname:', clientNickname.value);
    ...
    const submitProjectInfo = async () => {
      ...
      try {
        await router.push({
          path: '/createEst/infoAttributes', // Путь к странице attributesInfo.vue
          query: {
            clientName: clientInfo.clientName,
            clientNickname: clientInfo.clientNickname,
            PM: clientInfo.PM,
            milestone: clientInfo.milestone,
    ...
    const PlatformOptions = ref([
      { id: 1, label: 'Mobile' },
      { id: 2, label: 'Desktop' },
      { id: 3, label: 'Desktop + Mobile' },
      { id: 4, label: 'Plate Table' },
      { id: 5, label: 'Plate Table + Mobile' },
      { id: 6, label: 'Desktop + Plate Table' },
      { id: 7, label: 'Desktop + Plate Table + Mobile' },
    ]);
    ...
    const filteredOptions = computed(() => {
      ...
      return TypeOptions.value.filter(option =>
    ...
    const filteredOptionsMile = computed(() => {
      ...
      return PlatformOptions.value.filter(option =>
